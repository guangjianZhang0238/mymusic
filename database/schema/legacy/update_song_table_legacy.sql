...
ADD COLUMN artist_ids JSON NULL COMMENT '歌手ID数组，用于存储多个歌手' AFTER artist_id;

-- 添加 artist_names 字段（文本格式存储歌手名称）
ALTER TABLE content_song 
ADD COLUMN artist_names TEXT NULL COMMENT '歌手名称列表，用于展示' AFTER artist_ids;
...
-- 创建索引以提高查询性能
CREATE INDEX idx_content_song_artist_ids ON content_song((CAST(artist_ids AS CHAR(255))));
