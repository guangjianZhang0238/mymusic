...
fun PlaylistEditScreen(
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text("编辑歌单", color = Color.White) },
                navigationIcon = {
                ...
                Button(
                    onClick = {
                        viewModel.updatePlaylist(
                            ...
                            name = name,
                            coverImage = if (coverImage.isNotBlank()) coverImage else null,
                    ...
                    if (uiState.playlistsLoading) {
                        ...
                        Text("更新歌单", fontSize = 18.sp, fontWeight = FontWeight.Bold)
                    }
                }
                // 更新成功后自动返回
                LaunchedEffect(uiState.userPlaylists) {
                    // 简单检查：如果用户歌单列表已更新，则认为更新成功
